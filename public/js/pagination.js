const displayPagination = (data, currentPage) => {
  if (data.pages > 0) {
      let paginationButtons = ''
      let lastPage = data.pages
      ...
      $('.pagination div').click(function (e) {
          ...
          $.getJSON(`/api/all-hours/${clickedPageNumber}`)
          .then((newdata) => {
              loadAllHours(clickedPageNumber)
              displayPagination(newdata, clickedPageNumber);
          })
          .catch((err) => {
            console.log(err)
            // TODO: error review
